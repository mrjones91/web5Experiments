...
let { record } = await web5.dwn.records.create({
    data: 'Hello, Web5!',
    message: {
...
record  = await web5.dwn.records.create({
    data: 'Hackathon Winners: DJ, Jay, & Conner!',
    message: {
        dataFormat: 'text/plain'
    }
});

const readResult000 = await record.data.text();
console.log('read Result is ' + readResult000);
